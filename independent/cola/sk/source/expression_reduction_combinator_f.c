...
_BEGIN_DECLARATION_
result_t expression_reduction_combinator_f(expression_t* expression, index_t* root)
_END_DECLARATION_
{
  if
  (
    expression_compare_name(expression, *root, COMBINATOR_F_NAME) == RESULT_OK &&
  ...
  )
  {
    ...
    expression_delete(expression, item_1_index);
    *root = item_2_index;
